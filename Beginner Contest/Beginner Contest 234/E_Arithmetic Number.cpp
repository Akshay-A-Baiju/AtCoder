...
// Problem link : 
// Contest : 
// Problem tags : precomputation, medium
// TC : O(1)
// SC : O(1)
...
void fun(string s, ll d)
{
	if (s.size()>=19)	// base condition, only reachable for "1111111111111111111"
	{
		st.ins(s);
		string str=s;
		reverse(all(str));
	...
	char bk=s.back();
	if (bk-'0'+d>9)		// an AP series is completed, hence add it and its reverse (for negative d) to set
	{
		st.ins(s);
		string str=s;
		reverse(all(str));
	...
	// else add AP series and its reverse to set, and keep building more terms
	st.ins(s);
	string str=s;
	reverse(all(str));
	st.ins(str);
...
void solve()
{
	// precompute the AP numbers
	st.ins("0");
	for (char ch='0';ch<='9';ch++)
	{
		for (ll d=0;d<=9;d++)
		{
			if (ch=='0' && d==0) continue;
			string s;
			s.pb(ch);
			fun(s,d);
		}
	...
	// binary search isnt really needed as no of strings are very less
	while (hi-lo>1)
	{
		ll mid=(hi+lo)/2;
		if (cmp(v[mid],s))
