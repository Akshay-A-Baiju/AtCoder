...
// Time Limit: 2000 ms
// Problem tag: dp, difference array
// TC: O(n*m)
// SC: O(n*m)
...
void solve()
{
	...
	for (ll i=1;i<n;i++)
	{
		...
		for (ll j=1;j<=m;j++)
		{
			...
			if (j-k>=1)
			{
				temp[1]=(temp[1]+dp[j])%MM;
				temp[j-k+1]=(temp[j-k+1]-dp[j]+MM)%MM;
			}
			// right -> range [s,e]
			if (j+k<=m)
			{
				temp[j+k]=(temp[j+k]+dp[j])%MM;
				temp[m+1]=(temp[m+1]-dp[j]+MM)%MM;
			}			
		}
		for (ll j=1;j<m+7;j++) temp[j]=(temp[j]+temp[j-1])%MM;
		dp=temp;
	}
	ll ans=0;
	for (ll i=1;i<=m;i++) ans=(ans+dp[i])%MM;
	cout<<ans;
}
