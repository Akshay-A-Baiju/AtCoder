...
// URL: https://atcoder.jp/contests/abc291/tasks/abc291_d
...
// ********     ACCEPTED     ********
ll fun(ll ind, ll flip, vector <pll> &v, vvll &dp)
{
	if (ind==0) return 1;
	if (dp[ind][flip]!=-1) return dp[ind][flip];
	ll face=v[ind].ff,back=v[ind].ss;
	if (flip) swap(face,back);
	ll ans=0;
	if (ind-1>=0 && v[ind-1].ff!=face) ans=fun(ind-1,0,v,dp);
	if (ind-1>=0 && v[ind-1].ss!=face) ans=(ans+fun(ind-1,1,v,dp))%MM;
	return dp[ind][flip]=ans;
}
...
void solve()
{
    ...
    vvll dp(n, vll (2,-1));
    ll ans=fun(n-1,0,v,dp);
    ans=(ans+fun(n-1,1,v,dp))%MM;
    cout<<ans;
}
