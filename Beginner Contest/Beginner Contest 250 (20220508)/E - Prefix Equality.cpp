...
void solve()
{
	...
	set <ll> sta,stb;
	vll pfa(n,0),pfb(n,0);
	vll mna(n,inf),mnb(n,inf),mxa(n,0),mxb(n,0),suma(n,0),sumb(n,0);
	vector <ld> meana(n,0),meanb(n,0);
	for (ll i=0;i<n;i++)
	{
		if (i==0)
		{
			mna[i]=mxa[i]=meana[i]=suma[i]=a[i];
			mnb[i]=mxb[i]=meanb[i]=sumb[i]=b[i];
			sta.ins(a[i]);
			stb.ins(b[i]);
		}
		else
		{
			...
			if (sta.find(a[i])==sta.end())
			{
				suma[i]+=a[i];
				sta.ins(a[i]);
				meana[i]=1.0*suma[i]/sta.size();
			}
			if (stb.find(b[i])==stb.end())
			{
				sumb[i]+=b[i];
				stb.ins(b[i]);
				meanb[i]=1.0*sumb[i]/stb.size();
			}
		}
		pfa[i]=sta.size();
		pfb[i]=stb.size();
	...
	{
		ll x,y; cin>>x>>y; x--; y--;
		if (pfa[x]==pfb[y] && mna[x]==mnb[y] && mxa[x]==mxb[y] && suma[x]==sumb[y] && abs(meana[x]-meanb[y])<eps) cout<<"Yes";
		else cout<<"No";
		cout<<endl;
	}
}
